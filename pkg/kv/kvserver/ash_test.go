...
func TestASHLockContention(t *testing.T) {
	...
	runner.Exec(t, `SET CLUSTER SETTING sql.ash.enabled = true`)
	runner.Exec(t, `SET CLUSTER SETTING sql.ash.sample_interval = '10ms'`)
	...
	// Give the blocked goroutine time to enter the lock wait queue and
	// allow ASH samples to be taken.
	...
	// Check for LockWait samples directly from the global sampler.
	samples := ash.GetSamples()
	var lockWaitCount int
	for _, s := range samples {
		if s.WorkEvent == "LockWait" {
			lockWaitCount++
		}
	}
	require.Greaterf(t, lockWaitCount, 0,
		"expected at least one ASH sample with LockWait event, "+
			"got %d total samples", len(samples))
}
