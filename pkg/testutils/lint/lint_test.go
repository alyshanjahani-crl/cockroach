...
func TestLint(t *testing.T) {
	...
	t.Run("TestASHInstrumentationCount", func(t *testing.T) {
		t.Parallel()

		// Count the number of ash.SetWorkState call sites in the codebase.
		// This test ensures that ASH instrumentation is not accidentally
		// added or removed without review. If this count changes, update
		// the expected value below after confirming the change is intentional.
		...
		if err := stream.ForEach(stream.Sequence(
			filter,
			// Exclude comment lines (e.g., doc.go examples).
			stream.GrepNot(`:\s*//`),
		), func(s string) {
			count++
		}); err != nil {
		...
		if count != expectedCount {
			t.Errorf(
				"expected %d ash.SetWorkState call sites, found %d; "+
					"if this change is intentional, update the expected "+
					"count in this test",
				expectedCount, count,
			)
		}
