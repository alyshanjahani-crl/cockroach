...
func TestJobWorkloadIDContextPropagation(t *testing.T) {
	...
	// Verify that the background context has no workload ID.
	require.Equal(t, uint64(0), kv.WorkloadIDFromContext(ctx))
		...
		func(_ *jobs.Job, _ *cluster.Settings) jobs.Resumer {
			return jobstest.FakeResumer{
				OnResume: func(ctx context.Context) error {
					observedWID.Store(kv.WorkloadIDFromContext(ctx))
					close(resumeDone)
					return nil
				},
			}
