...
// workload ID (via kv.ContextWithWorkloadID), EvalCtx.WorkloadID is
...
func MakeJobExecContext(
	...
	if wid := kv.WorkloadIDFromContext(ctx); wid != 0 {
		p.extendedEvalCtx.WorkloadID = wid
	}
...
// kv.ContextWithWorkloadID before calling the resumer. Any DB or
