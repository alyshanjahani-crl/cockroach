...
type WorkState struct {
	...
	WorkEvent string
	// EnteredAtNanos is when this state was entered (unix nanos).
	EnteredAtNanos int64
}
...
// Example usage:
//
//	cleanup := ash.SetWorkState("fingerprint123", ash.WORK_KV, "dist_sender_batch")
//	defer cleanup()
//	// ... perform work ...
func SetWorkState(workloadID uint64, eventType WorkEventType, event string) func() {
	...
	case IntentResolutionWorkloadID:
		state.WorkloadID = "intent resolution"
	case TxnHeartbeatWorkloadID:
		state.WorkloadID = "txn heartbeat"
	case RangeLookupWorkloadID:
		state.WorkloadID = "range lookup"
	case LeaseRequestWorkloadID:
		state.WorkloadID = "lease request"
	default:
		// TODO(alyshan): Dupe from sqlstats/persistedsqlstats/sqlstatsutil/json_encoding.go, refactor.
		state.WorkloadID = EncodeStmtFingerprintIDToString(workloadID)
	}
	state.WorkEventType = eventType
	state.WorkEvent = event
	state.EnteredAtNanos = timeutil.Now().UnixNano()
